...
var (
	...
	// MasterDbHostFlag MasterDb Flags (可选，如果未配置则跳过数据库连接)
	MasterDbHostFlag = &cli.StringFlag{
		Name:    "master-db-host",
		Usage:   "The host of the master database (optional, skip database if not set)",
	...
	MasterDbNameFlag = &cli.StringFlag{
		Name:    "master-db-name",
		Usage:   "The db name of the master database (optional, skip database if not set)",
